...
$query1 = mysqli_fetch_array(mysqli_query($con,"select * from se_discount where discount_id=$did"));
$pid=$query1['product_id'];
$query = mysqli_fetch_array(mysqli_query($con,"select * from se_product where product_id=$pid"));
//$q = mysqli_fetch_array(mysqli_query("Select * from se_discount WHERE product_id='$pid'"));
$dis=$query1['product_discount_percentage'];
echo "<body onload='updateText($dis)'>";
?>
		...
		<div class="box">
			...
			<div class="box-content">
				<h4 class="page-header">Update Discount</h4>
                                <form class="form-horizontal" role="form" method="post" action="Update_Product_Discount_Handler.php" name='profile_form'>
                                    
                                    <div class="form-group has-success has-feedback">
						<label class="col-sm-2 control-label">Product ID</label>
						<div class="col-sm-4">
							<?php echo "<input type='text' class='form-control' name='admin_id' value=".$query['product_id']." readonly>";?>
						</div>
					</div>
					...
					<div class="form-group has-success has-feedback">
						...
						<div class="col-sm-4">
							...
							<?php echo "<input type='Double' class='form-control' name='disc' value=".$query1['product_discount_percentage']." onchange=updateText(this.value)>";?>
						</div>
					</div>
...
//                                                    $q = mysqli_fetch_array(mysqli_query("Select * from se_discount WHERE product_id='$pid'"));
                                                    ?>
							<?php echo "<input type='Date' class='form-control' name='discfrom' value=".$query1['valid_from'].">";?>
						</div>
					</div>
                                    <div class="form-group has-success has-feedback">
...
//                                                    $q = mysqli_fetch_array(mysqli_query("Select * from se_discount WHERE product_id='$pid'"));
                                                    ?>
							<?php echo "<input type='Date' class='form-control' name='discto' value=".$query1['valid_to'].">";?>
						</div>
					</div>
